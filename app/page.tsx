...
export default function Home() {
  ...
  return (
    <main>
      <header className="site-header"><Brand /><nav className="desktop-nav" aria-label="Primary navigation">{nav.map(([label,id]) => <a key={id} href={`#${id}`}>{label}</a>)}</nav><a className="button button-small header-cta" href={WA_LINK} target="_blank" rel="noreferrer">Book a session <Arrow /></a><button className={`menu-toggle ${menuOpen ? "open" : ""}`} aria-label="Toggle navigation" aria-expanded={menuOpen} onClick={() => setMenuOpen(!menuOpen)}><i/><i/></button><div className={`mobile-menu ${menuOpen ? "open" : ""}`}><nav aria-label="Mobile navigation">{nav.map(([label,id], index) => <a key={id} href={`#${id}`} onClick={() => setMenuOpen(false)}><span>0{index+1}</span>{label}</a>)}</nav><a className="button" href={WA_LINK} target="_blank" rel="noreferrer">Book a session <Arrow /></a></div></header>

      <section className="hero" id="home"><img className="hero-image" src="https://images.unsplash.com/photo-1503951914875-452162b0f3f1?auto=format&fit=crop&w=2200&q=90" alt="Barber making a precision haircut in a dark, refined studio" /><div className="hero-overlay" /><div className="hero-content"><p className="eyebrow hero-eyebrow"><span /> Premium barbershop · Est. 2026</p><h1>More than a haircut.<br/><em>A standard.</em></h1><div className="hero-bottom"><p>Precision cuts, timeless style and a premium grooming experience crafted for the modern gentleman.</p><div className="hero-actions"><a className="button button-light" href={WA_LINK} target="_blank" rel="noreferrer">Book your appointment <Arrow /></a><a className="text-link" href="#services">View services <span>↓</span></a></div></div></div><div className="hero-meta"><div><span>MON — SAT</span><strong>09:00 — 20:00</strong></div><div><span>LOCATION</span><strong>LIMA, PERU</strong></div><span className="scroll-cue">SCROLL <i/></span></div></section>
